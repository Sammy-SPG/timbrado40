...
///////////////////////////////////////////////////////////////////////////////
function imprime_factura($xml_archivo,$titulo,$tipo_factura,$logo,$nota_impresa,$color_marco,$color_marco_texto,$color_texto,$fuente_texto,$modo)
{
...
*/
    ...
    if($version=="4.0")
    {
        foreach ($xml->xpath('//cfdi:Comprobante//cfdi:InformacionGlobal') as $InformacionGlobal)
        {
            $Periodicidad=$InformacionGlobal['Periodicidad'];
            $Meses=$InformacionGlobal['Meses'];
            $Ano__=$InformacionGlobal['A�o'];
            $html_InformacionGlobal="<br>INFORMACION GLOBAL: Periodicidad:$Periodicidad Meses: $Meses A�o: $Ano__<br>";
        }
    }
    ...
    foreach ($xml->xpath('//cfdi:Comprobante//cfdi:Emisor') as $Emisor)
    {
       ...
       $regimen_fiscal=$Emisor['RegimenFiscal'];
        if($regimen_fiscal != "")
        {
            $regimen_fiscal=formato_regimen_fiscal_modulo($regimen_fiscal);
            $regimen_fiscal_txt="Regimen Fiscal: $regimen_fiscal <br/>";
        }
...
$expedido_localidad=autoformato_impresion_modulo($ExpedidoEn['localidad']);
    ...
    $NumRegIdTrib=$Receptor['NumRegIdTrib'];
        ...
        if($DomicilioFiscalReceptor != "")
            $DomicilioFiscalReceptor_txt="CP Fiscal: $DomicilioFiscalReceptor <br/>";
        
        $RegimenFiscalReceptor=$Receptor['RegimenFiscalReceptor'];
        if($RegimenFiscalReceptor != "")
        {
            $RegimenFiscalReceptor=formato_regimen_fiscal_modulo($RegimenFiscalReceptor);
            $RegimenFiscalReceptor_txt="Regimen Fiscal: $RegimenFiscalReceptor <br/>";
        }
    ...
    if($version=='4.0'){
        $desgloce='<table width="100%">
           ...
           <td width="44px">CNT</td>
            ...
            <td   width="100px" align="right">PRECIO UNITARIO</td>
            <td   width="100px"  align="right">IMPORTE</td>
            <td   width="100px"  align="right">OBJETO IMPUESTO</td>
           </tr>
...
/*
    ...
    */
    ...
    //ESCUELAS
    foreach ($xml->xpath('//cfdi:Comprobante//cfdi:Conceptos//cfdi:Concepto//cfdi:ComplementoConcepto//iedu:instEducativas ') as $instEducativas)
    {
        $nombre_alumno= $instEducativas['nombreAlumno'];
        $curp_alumno=$instEducativas['CURP'];
        $nivel_educativo=$instEducativas['nivelEducativo'];
        $autRVOE=$instEducativas['autRVOE'];
        $rfc_pago_aumno=$instEducativas['rfcPago'];
        
        $html_complemento_escuelas="
        
        <br>
        <div>
            <div>Nombre del alumno: $nombre_alumno </div>
            <div>CURP: $curp_alumno </div>
            <div>Nivel Educativo: $nivel_educativo </div>
            <div>autRVOE: $autRVOE </div>
            <div>RFC Pago: $rfc_pago_aumno </div>
        </div>
        ";
    ...
    foreach ($xml->xpath('//cfdi:Comprobante//cfdi:Conceptos//cfdi:Concepto') as $Concepto)
    {
        ...
        {  
            ...
            if($TipoDeComprobante != 'P')
            {
                ...
                <td>$CveUnidad </td>
                <td>$unidad </td>
                <td>$descripcion $html_complemento_escuelas</td>
                <td align='right'>$$precio_unitario_</td>
                <td  align='right'>$$importe_</td>
                ";  
                if($version=='4.0')
                {
                 $desgloce.="<td  align='right'>$ObjetoImp</td>";  
                 }
                $desgloce.="</tr>";
    ...
    if($version=='4.0')
    {
        foreach ($xml->xpath('/cfdi:Comprobante/cfdi:Impuestos/cfdi:Traslados/cfdi:Traslado') as $TrasladoC){
            $Base=$TrasladoC['Base'];         //COMPARA SI ES IMPUESTO DE PRODUCTO O DE COMPROBANTE
            $tasa=$TrasladoC['TasaOCuota'];
            $importe=$TrasladoC['Importe'];
            $importe_=number_format((string)$importe,2);
            $impuesto= $TrasladoC['Impuesto'];
            $impuesto_txt=formato_impuestos_modulo($impuesto);
            $TipoFactor= $TrasladoC['TipoFactor'];
            //if($Base =='')
            //{
                $total_translados=$total_translados+(float)$importe;
                $tasa_txt=doubleval($tasa)*100;
...
$trMercancias.="
    <tr class='factura_detalles_cabecera $class'>
        ...
        <td colspan='2'>FraccionArancelariffffff</td>
...
";
$html_comercion_exterior=""; //SI FUNCIONA, SOLO SE QUITO
//////////  DISEÑO ////////////
...
if($version=='3.3' OR $version=='4.0')
{
    ...
    <div class='factura_receptor factura_cuadro '>
        ...
        Uso CFDI: $uso_CFDi
  $rfc_extranjero
        $RegimenFiscalReceptor_txt
        $DomicilioFiscalReceptor_txt
    </div>
    ";
}
...
//SI SE UNA INI, SE CONVIERTE A BASE64 LA IMAGEN
if($modo == 'INI')
{
    if(file_exists($logo))
    {
        $contenidoBinario = file_get_contents($logo);
        $logoB64 = base64_encode($contenidoBinario);
        $ruta_logo ="data:image/png;base64,$logoB64";
...
else
{
    ...
    $numeroletras=num2letras_kit($total,'  ');    
...
if($modo == 'INI')
{
    ...
    {
        $contenidoBinario = file_get_contents($archivo_png);
        $archivo_pngB64 = base64_encode($contenidoBinario);
        $ruta_qr ="data:image/png;base64,$archivo_pngB64";
...
///////
function formato_regimen_fiscal_modulo($regimen)
{
    $regimen=str_replace('601','General de Ley Personas Morales (601)',$regimen);
    $regimen=str_replace('603','Personas Morales con Fines no Lucrativos (603)',$regimen);
    $regimen=str_replace('605','Sueldos y Salarios e Ingresos Asimilados a Salarios (605)',$regimen);
    $regimen=str_replace('606','Arrendamiento (606)',$regimen);
    $regimen=str_replace('607','Regimen de Enajenaci�n o Adquisici�n de Bienes (607)',$regimen);
    $regimen=str_replace('608','Demas ingresos (608)',$regimen);
    $regimen=str_replace('610','Residentes en el Extranjero sin Establecimiento Permanente en M�xico (610)',$regimen);
    $regimen=str_replace('611','Ingresos por Dividendos (socios y accionistas) (611)',$regimen);
    $regimen=str_replace('612','Personas F�sicas con Actividades Empresariales y Profesionales (612)',$regimen);
    $regimen=str_replace('614','Ingresos por intereses (614)',$regimen);
    $regimen=str_replace('615','Regimen de los ingresos por obtencion de premios (615)',$regimen);
    $regimen=str_replace('616','Sin obligaciones fiscales (616)',$regimen);
    $regimen=str_replace('620','Sociedades Cooperativas de Producci�n que optan por diferir sus ingresos (620)',$regimen);
    $regimen=str_replace('621','Incorporaci�n Fiscal (621)',$regimen);
    $regimen=str_replace('622','Actividades Agr�colas, Ganaderas, Silv�colas y Pesqueras (622)',$regimen);
    $regimen=str_replace('623','Opcional para Grupos de Sociedades (623)',$regimen);
    $regimen=str_replace('624','Coordinados (624)',$regimen);
    $regimen=str_replace('625','R�gimen de las Actividades Empresariales con ingresos a trav�s de Plataformas Tecnol�gicas (625)',$regimen);
    $regimen=str_replace('626','R�gimen Simplificado de Confianza (626)',$regimen);
